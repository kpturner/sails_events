...
module.exports = {
    ...
    if (res.locals.user && res.locals.user.username.toLowerCase()=="rwilson") {
      if (sails.config.events.developer) {
          sails.hooks.email.send(
          "diagnostic", {
                err: "rwilson validating request"                      
              },
              {
    ...
    if (!req.session.authenticated) {
      ...
      if (sails.config.events.developer) {
          sails.hooks.email.send(
          "diagnostic", {
                err: "rwilson session not authenticated"                      
              },
              {
                to: sails.config.events.developer,
                subject: "rwilson checking"
