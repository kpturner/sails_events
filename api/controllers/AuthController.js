...
var AuthController = {
            ...
            Token.destroy({ user: user.id }, function () {
              ...
              Token.create({ token: token, user: user.id }, function (err) {
                ...
                res.cookie('remember_me', token, {
                  path: '/',
                  httpOnly: true,
                  maxAge: sails.config.passport.rememberme.maxAge
                }); // 7 days
                // Upon successful login, send the user to the default page
                res.redirect('/');
              });
  ...
  updateProfile: function (req, res) {
    ...
    User.findOne(req.user.id)
      .populate('passports')
      .exec(function foundUser(err, currentUser) {
        ...
        function updateUser(delta) {
          User.update(req.user.id, delta).exec(function afterwards(err, updatedUser) {
            ...
            if (!updatedUser[0].isVO) updatedUser[0].isVO = false;
            if (!updatedUser[0].isDC) updatedUser[0].isDC = false;
            if (!updatedUser[0].isAdmin) updatedUser[0].isAdmin = false;
            if (!updatedUser[0].isOrganiser) updatedUser[0].isOrganiser = false;
            if (updatedUser[0].authProvider != 'local') updatedUser[0].username = 'N/A';
            var orders = [];
        ...
        Passport.findOne({ user: user.id }).exec(function (err, passport) {
          if (passport) {
            ...
            if (passport.provider) {
              resetInstructions =
                'You originally registered using your ' +
                passport.provider +
                ' account. To log on, simply click the ' +
                passport.provider +
