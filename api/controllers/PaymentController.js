...
module.exports = {
  ...
  getStripe: async (eventArg) => {
    const getStripeObject = (event) => {
      ...
      sails.log.debug(
        `Looking for online payment details for ${event.onlinePaymentPlatform} config ${event.onlinePaymentConfig}`
      );
    ...
    return new Promise((resolve, reject) => {
      try {
        if (userArg.id) {
          ...
          User.findOne(userArg).exec(function (err, user) {
            if (err) {
              return reject(err);
            }
            try {
            ...
            try {
              ...
              if (booking.amountPaid) {
                ...
                if (!Number.isInteger(quantity)) {
                  ...
                  sails.log.debug(
                    `Sticking with an integer amount of ${amount} and a quantity of 1 because the balance divided by ${booking.event.price} is not an integer.`
                  );
                } else {
    ...
    setTimeout(async function () {
      ...
      Email.send(
        ...
        },
        {
          ...
          to: sails.controllers.booking.bookingBCC(booking, [
            event.organiser,
            event.organiser2,
  ...
  getRefundMap: (booking, paymentMap, refundAmount) => {
    ...
    if (amountLeft) {
      sails.log.error(
        `Cannot issue refund for booking ${booking.id}, amount: ${refundAmount} as the booking payment references do not cover the refund amount`
      );
      return null;
    }
    return refundMap;
    ...
    try {
      if (booking.paymentReference) {
        ...
        try {
          ...
          sails.log.error(
            `Cannot issue refund for booking ${booking.id}, amount: ${amount} as the booking payment reference ${booking.paymentReference} is in the wrong format`
          );
          return null;
        }
        ...
        if (ok) {
          try {
            Booking.update(booking.id, {
              ...
              sails.log.debug(
                `Successfully updated refund details on booking ${booking.id} amount ${amount}. Sending confirmation email.`
              );
              Email.send(
                'onlineRefundSuccess',
                {
        ...
        sails.log.error(
          `Cannot issue refund for booking ${booking.id}, amount: ${amount} as the booking has no initial payment reference`
        );
      }
    } catch (err) {
      sails.log.error(err);
            ...
            Event.findOne(booking.event.id)
              ...
              .exec(async (err, event) => {
                if (err) {
                  ...
                  sails.log.debug(
                    `Successfully found event "${event.name}" (id: ${event.id}) for ${booking.ref} for session id ${req.query.session_id}`
                  );
                  const session = await sails.controllers.payment.getCheckoutSession(
                    req.query.session_id,
                    booking.event.id
                  );
                  if (session) {
                    sails.log.debug(
                      `Successfully fetched checkout session for ${req.query.session_id} (booking: ${booking.id} event: ${booking.event.id} paymentReference: ${booking.paymentReference})`
                    );
                  } else {
                  ...
                  if (session) {
                    if (!paymentMap[session.payment_intent]) {
                      sails.log.debug(
                        `Online payment - flagging booking ${booking.id} as paid etc ${JSON.stringify(session)}`
                      );
                      // Update the booking
                      ...
                      Booking.update(booking.id, {
                        ...
                        sails.log.debug(
                          `Successfully updated payment details on booking ${booking.id}. Sending confirmation email.`
                        );
                        Email.send(
                          'onlinePaymentSuccess',
                          {
                            ...
                            additionalInfo: event.recoverOnlinePaymentFee
                              ? 'NOTE: This payment includes a NON-REFUNDABLE online payment fee.'
                              : '',
                            booking,
                            event
                          },
                      ...
                      sails.log.warn(
                        `Online payment for booking ${booking.id} reference ${session.payment_intent} already processed!`
                      );
                    }
                  }
                }
