angular.module('EventsModule').controller('UserDetailsController', [
  '$scope',
  '$http',
  'toastr',
  'ngDialog',
  function ($scope, $http, toastr, ngDialog) {
    ...
    $http
      .get('/otherorders/' + SAILS_LOCALS.userDetails.id)
    ...
    $scope.detailsComplete = function () {
      var complete = true;
      if (
        !$scope.userdetailsForm.name ||
        $scope.userdetailsForm.name.length == 0 ||
        !$scope.userdetailsForm.salutation ||
        $scope.userdetailsForm.salutation.length == 0 ||
        ...
        ($scope.userdetailsForm.authProvider != 'dummy' &&
          ...
          (!$scope.userdetailsForm.lodge ||
            $scope.userdetailsForm.lodge.length == 0 ||
            !$scope.userdetailsForm.lodgeNo ||
            isNaN($scope.userdetailsForm.lodgeNo))) ||
        !$scope.userdetailsForm.surname ||
        $scope.userdetailsForm.surname.length == 0 ||
        !$scope.userdetailsForm.firstName ||
        $scope.userdetailsForm.firstName.length == 0 ||
      ...
      $http
        .get(
          '/user?_csrf=' +
            SAILS_LOCALS._csrf +
            '&where={"id":{"not":"' +
            encodeURIComponent(SAILS_LOCALS.userDetails.id.toString()) +
